...
(() => {
  ...
  if (!q) return false;
  if (q.type === "likert_scale" || q.type === "multiple_choice") {
    return Number.isInteger(a?.idx);
  }
...
// (If you already added likertLegend earlier, skip this to avoid duplicates.)
...
function bindNavForCurrent() {
  ...
  // Global Next handler — do not override inside render()
nextBtn.onclick = () => {
  const q = questions[index];
  ...
  // Build the list we’ll render (only supported types)
  questions = results
    .flatMap(j => Array.isArray(j.questions) ? j.questions : [])
    .filter(q => ["likert_scale", "numeric_scale", "multiple_choice"].includes(q?.type));

  // Remove safety/consent gate items and the premature "identify archetype" item
  const DROP_KEYWORDS = [
    "safety", "safe word", "safe-word", "protocol", "ssc", "rack",
    "consent", "aftercare", "negotiation", "injury", "emergency"
  ];
...
bindNavForCurrent();
  ...
  rawOpts.forEach((opt, i) => {
    ...
    btn.addEventListener("click", () => {
      answers[q.id] = { idx: i, value: i };
      if (navigator.vibrate) navigator.vibrate(15);
      render(); // this will re-run bindNavForCurrent()
    });
    qOptions.appendChild(btn);
  });
}
  ...
  input.addEventListener("input", () => {
    answers[q.id] = { value: Number(input.value) };
    out.textContent = String(input.value);
    bindNavForCurrent(); // enable Next once they touch the slider
  });

  wrap.appendChild(input);
  wrap.appendChild(out);
  qOptions.appendChild(wrap);
  return; // fine to return; Next is handled globally
}
 else if (q.type === "multiple_choice") {
  (q.response_options || []).forEach((opt, i) => {
    const btn = document.createElement("button");
